...
class TestNoteRepository:
    ...
    async def test_add(self, snapshot) -> None:
        ...
        # When
        note_list = await repository.add(account_entity=account_entity, note_value_list=[note])
        # Then
        assert jsonable_encoder(note_list) == snapshot(exclude=props('id_', 'created_at'))
